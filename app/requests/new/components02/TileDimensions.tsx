    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="grid gap-2 grid-cols-3">
        <div className="relative">
          <Input
            value={tileDistance}
            onChange={handletileDistanceChange}
            label="Ziegeldeckmaß"
          />
          {tileDistance.length != 0 && (
      ...
      <Select
        value={fixationType}
        onChange={(choice) => handleFixationTypeChange(choice as string)}
        label="Befestigung der Ziegel"
      >
      ...
      {["solidified", "cramped", "clued"].includes(fixationType) ? (
        <Alert color="blue" variant="outlined">
          <span>
            Mehrkosten in Höhe von 150,00 € zzgl. MwSt. pro KWp fallen an.
          </span>
        </Alert>
      ) : null}
      {fixationType == "unknown" ? (
        <Alert color="blue" variant="outlined">
          <span>Auftraggeber übernimmt mögliche Mehrkosten.</span>
        </Alert>
      ) : null}
      {["docked", "slate", "pantile"].includes(fixationType) ? (
        <Alert color="blue" variant="outlined">
